...
| INFO | `lease.denied` | `ns pod clabel gpus status reason retryable detail retry_s` | App refused the ask (409), or a transient network/5xx failure. `detail` is the app's reason — absent when the app never answered. On a 409, `reason`/`retryable` are the app's admission-denial envelope (`reason` is the denial code) and it is also mirrored to the pod as an `OnDemandLeaseDenied` Event. **`retryable=false`** is a structural denial the same ask can never pass (class GPU cap, not a member, group term over): it backs off exponentially to 30 min instead of retrying every 2–5 min, and its Event says waiting will not help. A contended denial retries in 2–5 min, or waits for the app's `not_before` (capped at 30 min); `retry_s` is the chosen delay. |
...
| INFO | `api.lease_denied` | `poduid status reason retryable detail` | Client-side view of a routine 409 denial, with the reason the app gave. Pairs with `lease.denied` above. |
